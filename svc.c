...
char *svc_commit(void *helper, char *message) {
        ...
        for (int j = 0; j < stage->tracked_file->size; j++) {
            ...
            if (new_file->file_path != NULL && tracked_file->file_path != NULL && strcmp(new_file->file_path, tracked_file->file_path) == 0) {
                if (new_file->hash != tracked_file->hash) {
                    ...
                    new_file->file_content = (unsigned char*)strdup((const char*)tracked_file->file_content);
    ...
    for (int i = 0; i < commit->commited_file->size; i++) {
        ...
        for (int j = 0; j < last_commit->commited_file->size; j++) {
            ...
            if (strcmp(new_file->file_path, last_new_file->file_path) == 0 && new_file->state == last_new_file->state) {
...
int svc_checkout(void *helper, char *branch_name) {
    ...
    for (int i = 0; i < commit->commited_file->size; i++) {
        ...
        if (file->state != REMOVED) {
            ...
            fwrite(file->file_content, 1, strlen((const char*)file->file_content), fp); //Restore all changes
        ...
        if (resolutions[i].resolved_file == NULL) {
            ...
            for (int j = 0; j < stage->tracked_file->size; j++) {
                ...
                if (file->file_path != NULL && strcmp(file->file_path, resolutions[i].file_name) == 0) {
                    ...
                    file_in_stage->file_content = (unsigned char*)strdup((const char*)file_contents);
                    fp = fopen(file_in_stage->file_path, "wb");//Write the content into previous files
                    fwrite(file_in_stage->file_content, 1, strlen((const char*)file_in_stage->file_content), fp);
                    fclose(fp);
